    ...
    <body>
        <h1>
            <a href="{{route('contatos.index')}}">
                @if(isset($contato))
                    SAC - Visualização do Contato
                @else
                    SAC - Criação de Contato
                @endif
            </a>
        </h1>
        @csrf
        ...
        <div class="card container">
            @if (isset($contato))
                <h2 class="card-header"> Contato: {{$contato->nome}} </h2>
            @endif
                <div class= "card container">
            ...
            <div class= "container">
                ...
                {!! Form::submit('Salvar', ['class' => 'btn form btn-success', $form??null]) !!}
                {!! Form::close() !!}

                @if (isset($contato))
                    {!! Form::open(['route' => ['contatos.destroy', $contato->id], 'method' => 'DELETE', 'name' => 'form'])!!}
                    {!! Form::submit('Excluir', ['class' => 'btn form btn-danger', $form??null]) !!}
                    {!! Form::close() !!}
                @endif
            </div>
        </div>
